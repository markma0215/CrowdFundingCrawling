...
class CompareProperty:
    ...
    baseField = ['Portal ID', 'Campaign ID', 'Campaign Name', 'Data Collection Date', 'First_Time(0/1)', 'Funded(0/1)',
                 'Date Posted', 'Date Funded', '% Funded', 'Funded', 'Expected']

    def setData(self, fundData, processData, fundField, procField):

        self.fundNameID = {fund['Campaign Name']: fund['Campaign ID'] for fund in fundData}
        self.processNameID = {process['Campaign Name']: process['Campaign ID'] for process in processData}
    ...
    def getID(self, name, category):
        if category == 'fund':
            if name in self.fundNameID:
                return self.fundNameID[name]
            else:
                return -1
        else:
            if name in self.processNameID:
                return self.processNameID[name]
            else:
                return -1

    def detectChanges(self, data, category):

        if data['First_Time(0/1)'] is not '1':
            for eachKey in data:
                if eachKey not in self.baseField:
                    data.update({eachKey: ''})
            ...
            if data['First_Time(0/1)'] is '1':
                ...
                for eachData in reversed(self.prevProcessData):
                    if data['Campaign ID'].strip() == eachData['Campaign ID']:
                        if data['% Funded'] != eachData['% Funded'] or (
                            data['Funded'] != eachData['Funded'] and
                            data['Expected'] != eachData['Expected']
                        ):
