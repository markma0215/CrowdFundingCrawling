

# global variables
base_url = "https://app.crowdstreet.com"
login_url = "https://app.crowdstreet.com/accounts/login/"
username = "rheazhou@gmail.com"
password = "Crowd1234566$$$"
session = ""
website_structure_has_changed = "website structure has changed"

current_campaign_id = 1
isFirstTime = False

First_Run_Funded = "Data Collection/1/1_First Run_Funded/Portal ID_1_First Run_Funded.csv"
First_Run_In_Progress = "Data Collection/1/1_First Run_In Progress/Portal ID_1_First Run_In Progress.csv"
Subsequent_Runs_Funded = "Data Collection/1/1_Subsequent Runs_Funded/Portal ID_1_Subsequent Runs_Funded_{date}.csv"
Subsequent_Runs_In_Progress = "Data Collection/1/1_Subsequent Runs_In Progress/Portal ID_1_Subsequent Runs_In Progress_{date}.csv"
Documents_In_Progress = "Data Collection/1/1_Documents_In Progress/1.{ID}/{file_name}"
